...
static uint8_t slaveAddr = I2C_CHIP_ADDR;
...
int MLX90640_IsDataReady(void) {
	uint16_t statusRegister = 0;
	if (MLX90640_I2CRead(slaveAddr, 0x8000, 1, &statusRegister) == 0 &&
		((statusRegister & 0x0008) != 0))
	{
		//statusRegister &= 0b1111111111110111; // reset ready
...
int MLX90640_StartMeasurement(void)
{
    uint16_t statusRegister;
    int err = MLX90640_I2CRead(slaveAddr, 0x8000, 1, &statusRegister);
    ...
    //statusRegister |= 0b0000000000100000; // Set b5: start of measurement //checkme почему так не работает(даже когда if_overwrite 0)?

    err |= MLX90640_I2CWrite(slaveAddr, 0x8000, statusRegister);
    return err;
}

int MLX90640_GetData(uint16_t *frameData)
{
    int error = 0;
    uint16_t statusRegister;
...
int MLX90640_GetFrameData(uint16_t *frameData)
{
    ...
    return dataReady; //dataReady д.б. равен 0
}
...
int MLX90640_GetControl(uint16_t *control)
{
    return MLX90640_I2CRead(slaveAddr, 0x800D, 1, control);
}
 ...
 */
int MLX90640_SetDataHold(uint8_t isDataHold)
{
    uint16_t controlRegister1;
    int value;
    int error;

    value = (isDataHold & 0x01)<<2;

    error = MLX90640_I2CRead(slaveAddr, 0x800D, 1, &controlRegister1);
    if(error == 0)
    {
        value = (controlRegister1 & 0b1111111111111011) | value;
        error = MLX90640_I2CWrite(slaveAddr, 0x800D, value);
    }
